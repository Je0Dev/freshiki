...
- Verify each feature (build clean, `cargo test` green) before moving on.
...
```text
feat: add SM-2 interval scheduler
fix: correct due-date handling at midnight
test: cover review grade transitions
...
2. Format, lint, and run all tests:
...
Note: tests are for **really important features** (SRS scheduling, persistence),
not for everything implemented. Small glue helpers and UI wrappers need no tests.